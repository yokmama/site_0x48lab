...
import styles from '../ai-dev/page.module.css'
...
const FIELDS: Record<TabKey, { key: string; label: string; multiline?: boolean; array?: boolean; type?: string }[]> = {
  problems: [
    { key: 'text', label: 'テキスト' },
  ],
  solutions: [
  ...
  results: [
    ...
    { key: 'title', label: 'タイトル' },
    { key: 'beforeCost', label: '改善前コスト' },
    { key: 'beforePeriod', label: '改善前期間' },
    { key: 'afterCost', label: '改善後コスト' },
    { key: 'afterPeriod', label: '改善後期間' },
    { key: 'saving', label: '削減効果' },
  ],
  faq: [
    { key: 'q', label: '質問' },
...
function formToPayload(form: Record<string, string>, tab: TabKey): Record<string, unknown> {
  ...
  for (const f of FIELDS[tab]) {
    out[f.key] = f.array ? form[f.key].split('\n').map((s: string) => s.trim()).filter(Boolean) : form[f.key]
  }
  return out
}
...
function RowEditor({ tab, row, onSaved, onCancel }: RowEditorProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setGlobalError('')
    try {
      const url = isNew ? `/api/admin/lp/${tab}` : `/api/admin/lp/${tab}/${row!.id}`
...
export function LpTabsClient({ data }: { data: LpData }) {
  ...
  async function handleDelete(id: number) {
    ...
    setDeleting(id)
    try {
      await fetch(`/api/admin/lp/${activeTab}/${id}`, { method: 'DELETE' })
      router.refresh()
  ...
  return (
    <div className={styles.page}>
      <h1 className={styles.heading}>LPページ管理</h1>
